...
/*
Этот ТЗ оказался прям сложным для меня. И если с добавлением времени все +/- понятно, то с тестами вообще нет
Понимаю, что что-то не так, учитывая, что я не делал абстрактный класс, как сказано в ТЗ, но по-другому никак не получается.
Подсказки в пачке тоже не особо помогли.
 */
...
public class TaskManagerTest {
    ...
    void updateEpicStatus() {
        ...
        taskManager.addNewSubtask(new Subtask("subtask1", "subtaskDescription", Status.NEW, 3,
                1, 15, LocalDateTime.of(2022, 1, 1, 1, 1)));
    ...
    void getPrioritizedTasks() {
        ...
        assertEquals(set.toArray()[0], task, "Incorrect task position in prioritizedTasks");
        assertEquals(set.toArray()[1], subtask, "Incorrect task position in prioritizedTasks");
    }

    @Test
    void loadFromFile() {
        File save = new File("resources/saveFile.csv");
        TaskManager manager = FileBackedTasksManager.loadFromFile(save);
        ...
        taskManager.deleteSubtasks();
        save = new File("resources/saveFile.csv");
